...
<template>
  <div class="p-6 space-y-6">
    <div class="text-center py-2">
      ...
      <p class="text-xs text-slate-500 mt-1">간단한 문항을 통해 본인의 피싱 위협 취약성을 평가합니다.</p>
    </div>
    ...
    <div v-if="!store.result" class="space-y-6">
      ...
      >
        ...
        <div class="grid grid-cols-1 gap-2.5">
          <button 
            ...
            :class="[
              'w-full text-left text-xs py-3 px-4 rounded-xl border transition-all duration-200 active:scale-[0.98]',
      ...
      <button 
        ...
        class="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-slate-100 disabled:text-slate-400 disabled:shadow-none disabled:cursor-not-allowed text-white font-bold py-3.5 px-4 rounded-xl shadow-[0_4px_12px_rgba(37,99,235,0.2)] transition-all active:scale-[0.98] flex items-center justify-center gap-2 text-sm"
      >
    ...
    <div v-else class="bg-white border border-slate-200/85 rounded-2xl p-5 space-y-5 shadow-lg shadow-slate-100/50">
      <div class="text-center pb-3 border-b border-slate-100">
        ...
        <h3 class="text-md font-bold text-slate-800 mt-3">{{ store.result.riskType }}</h3>
      </div>

      <div class="space-y-4">
        <div>
          <h4 class="text-xs font-bold text-slate-400">분석 내용</h4>
          <p class="text-xs text-slate-600 mt-1 leading-relaxed">{{ store.result.description }}</p>
        </div>

        <div>
          <h4 class="text-xs font-bold text-slate-400">본인에게 취약한 범죄 수법</h4>
          <ul class="mt-2 space-y-2">
            <li 
              v-for="(vuln, idx) in store.result.vulnerabilities" 
              :key="idx"
              class="text-xs text-slate-650 flex items-start gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-100"
            >
